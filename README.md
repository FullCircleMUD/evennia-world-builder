...
- **Three-pass build:** non-exits in pass 1, exits in pass 2 (with destinations resolved against the just-built rooms), then pass 3 walks each file's `incoming_exits:` registry and restores any cross-file dependent exits that were missing — fetching their canonical files via the Reader and rebuilding them.
