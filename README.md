...
> **Identity is mid-migration.** The bullets below describe the shipped `(deployment_file, deployment_id)` machinery. The agreed replacement — a per-file `file_id` and a globally unique per-entity `entity_id`, neither derived from a path — is specified in [docs/deployment-identity.md](docs/deployment-identity.md) and already applied to the test-yaml fixtures. These bullets are updated as each part of the machinery converts.
